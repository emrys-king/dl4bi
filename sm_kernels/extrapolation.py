"""
SM-DeepRV EXTRAPOLATION Experiment
====================================
Companion script to simulation.py, focused on TRUE extrapolation rather
than interpolation. Held-out points lie outside the convex hull of the
observed points on at least one side (default: one corner quadrant,
~25% of the grid) — the regime classical GPs handle distinctively
(smooth reversion to the prior mean away from data, with growing
uncertainty), as opposed to the in-fill prediction tasks in
simulation.py where held-out points are largely surrounded by
observations.
...
Two extrapolation regions are supported via --edge_region:
  corner : one corner quadrant held out — extrapolation in 2 directions
           (the cleaner, harder test; default)
  side   : one full edge strip held out — extrapolation in 1 direction

Usage
-----
    # Train then run extrapolation for all configs (corner held-out, default)
    CUDA_VISIBLE_DEVICES=0 python extrapolation.py --grid_size 16
    ...
    # Edge-strip extrapolation instead of corner
    CUDA_VISIBLE_DEVICES=0 python extrapolation.py --grid_size 16 \
        --load_ckpt results/sim_checkpoints_grid16_q1_blks2_emb64 \
        --sm_config slow --edge_region side

    # Skip exact GP baseline (faster development)
    CUDA_VISIBLE_DEVICES=0 python extrapolation.py --grid_size 16 --no_exact_gp
"""
...
def parse_args():
    ...
    # Masking
    p.add_argument("--mask_type",     type=str,   default="edge",
                   choices=["edge", "contiguous", "random", "blob"],
                   help="Masking strategy. This script defaults to 'edge' "
                        "since it targets TRUE EXTRAPOLATION — held-out "
                        "points outside the convex hull of observations on "
                        "at least one side — rather than interpolation. "
                        "The other mask types (all of which place gaps "
                        "largely surrounded by observed points) are "
                        "retained for direct comparability with the main "
                        "prediction-task script.")
    p.add_argument("--edge_region",   type=str,   default="corner",
                   choices=["corner", "side"],
                   help="For --mask_type edge: 'corner' holds out one "
                        "corner quadrant (extrapolation in 2 directions — "
                        "the cleaner test), 'side' holds out one full edge "
                        "strip (extrapolation in 1 direction).")
    p.add_argument("--mask_fraction", type=float, default=0.25,
                   help="Fraction of grid to HOLD OUT (default: 0.25). "
                        "For edge/contiguous/blob this controls the "
                        "held-out region's size.")
    # Misc
...
def edge_mask(grid_size, mask_fraction=0.25, edge="corner", rng_key=None):
    """TRUE extrapolation mask: holds out a contiguous region touching one
    or more grid boundaries, so the held-out points lie OUTSIDE the convex
    hull of the observed points on at least one side — genuine
    extrapolation, as opposed to interpolation (contiguous_mask/blob_mask
    place gaps that are typically surrounded by observations on most/all
    sides).

    Args:
        grid_size:     number of grid points per side
        mask_fraction: fraction of grid to hold out (default 0.25 — a
                       quarter of the domain, e.g. one full corner quadrant)
        edge:          which boundary region to hold out:
                         "corner" — one corner quadrant (default; the
                                    cleanest extrapolation test, held-out
                                    region is fully outside the hull of
                                    observed points in 2 directions)
                         "side"   — one full edge strip (held-out region
                                    outside the hull in 1 direction)
        rng_key:       JAX random key (used only to randomise WHICH
                       corner/side is held out, for variation across seeds)
    """
    H = W = grid_size
    n_masked = int(H * W * mask_fraction)
    seed     = int(jax.random.bits(rng_key)) if rng_key is not None else 0
    rng_np   = np.random.default_rng(seed)
    mask     = np.zeros((H, W), dtype=bool)   # True = held out (test)

    if edge == "corner":
        # square corner block, area ~= n_masked
        side = max(1, int(round(np.sqrt(n_masked))))
        side = min(side, H, W)
        corner = rng_np.integers(0, 4)  # 0=BL 1=BR 2=TL 3=TR
        if corner == 0:   mask[:side, :side]  = True
        elif corner == 1: mask[:side, -side:] = True
        elif corner == 2: mask[-side:, :side] = True
        else:             mask[-side:, -side:]= True
        corner_name = ["bottom-left", "bottom-right",
                       "top-left", "top-right"][corner]
        log.info(f"Edge mask (corner): {side}×{side} {corner_name} "
                 f"quadrant held out")
    elif edge == "side":
        # full-width/height strip along one randomly chosen edge
        strip_rows = max(1, int(round(n_masked / W)))
        strip_rows = min(strip_rows, H)
        which = rng_np.integers(0, 4)  # 0=bottom 1=top 2=left 3=right
        if which == 0:   mask[:strip_rows, :]  = True
        elif which == 1: mask[-strip_rows:, :] = True
        elif which == 2: mask[:, :strip_rows]  = True
        else:            mask[:, -strip_rows:] = True
        side_name = ["bottom", "top", "left", "right"][which]
        log.info(f"Edge mask (side): {strip_rows}-row/col strip on "
                 f"{side_name} edge held out")
    else:
        raise ValueError(f"Unknown edge type: {edge!r} (use 'corner' or 'side')")

    flat    = mask.ravel()
    tst_idx = jnp.array(np.where( flat)[0])
    obs_idx = jnp.array(np.where(~flat)[0])
    log.info(f"Edge mask: {len(tst_idx)}/{H*W} held out "
             f"({100*len(tst_idx)/(H*W):.1f}%) — true extrapolation "
             f"(outside observed convex hull)")
    return obs_idx, tst_idx
...
def generate_observations(s_all, grid_size, sm_config, noise_std, rng,
                           mask_type="edge", mask_fraction=0.25,
                           edge_region="corner"):
    ...
    if mask_type == "edge":
        obs_idx, tst_idx = edge_mask(grid_size,
...
def main():
    ...
    run_name  = f"extrap_{timestamp}_grid{args.grid_size}"
